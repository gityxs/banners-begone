...
function Unistroke(name, points) // constructor
{
    ...
    this.Points = Resample(this.OriginalPoints, NumPoints);
...
function DollarRecognizer() // constructor
{
    ...
    this.removeUnistroke = function(name) {
        for (index = this.allUnistrokes.length - 1; index >= 0; index--) {
            let unistroke = this.allUnistrokes[index];
            if (unistroke.Name == name) {
                this.Unistrokes.splice(index, 1)
                return;
            }
        }
    }
    ...
    this.Recognize = function (points, useProtractor) {
        if (this.Unistrokes.length <= 0) return "Dollar: No Unistrokes unlocked";
        if (points.length <= 0) return "Dollar: No points sent";
